...
6. Run the remaining numbered migrations in `sql/` in order, including `sql/018_document_workflow.sql` for physical document receipt/delivery tracking, `sql/019_gis_issues_noauth_rls.sql` for GIS Issues access in the no-login application, `sql/020_gis_issue_delete.sql` for parent deletion with activity cascade, `sql/021_outage_job_responsible_unit.sql` for responsible-unit metadata, and `sql/022_outage_job_customer_count.sql` for nullable affected-customer counts. The migrations are idempotent and preserve existing records except when a user explicitly deletes a GIS Issue.
