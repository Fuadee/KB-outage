  ...
  return (
    <Modal isOpen={open} onClose={onClose} title={title} panelClassName="max-w-2xl">
      <div className="grid gap-3">
        <Input type="text" value={item.company_name} onChange={(event) => onChange({ company_name: event.target.value })} placeholder="ชื่อรายการ / ชื่อลูกค้า" className={getError("company_name") ? "border-red-400" : undefined} />
        {getError("company_name") ? <p className="text-xs text-red-400">{getError("company_name")}</p> : null}
        <Input type="text" value={item.contact_name ?? ""} onChange={(event) => onChange({ contact_name: event.target.value })} placeholder="ผู้รับผิดชอบ" />
        <Input type="text" value={item.note ?? ""} onChange={(event) => onChange({ note: event.target.value })} placeholder="หมายเหตุ" />
        ...
        <Input type="url" value={item.map_link ?? ""} onChange={(event) => onChange({ map_link: event.target.value })} placeholder="Google Maps link" className={getError("map_link") ? "border-red-400" : undefined} />
        {getError("map_link") ? <p className="text-xs text-red-400">{getError("map_link")}</p> : null}
